...
import godot.core.printWarning
...
fun <T: godot.GodotObject> constructFromRawMem(mem: COpaquePointer?, cons: KFunction0<T>): COpaquePointer? {
    if (mem == null) {
        printError("Invoked object constructor with null as argument", "kotlin constructor", "Entry.kt", 0)
    ...
    try {
        ...
        printError(e.message.toString(), "kotlin constructor", "Entry.kt", 0)
        e.printStackTrace()
        exitProcess(EXIT_FAILURE)
    }
}
...
inline fun <reified T: godot.GodotObject> deconstructFromRawMem(mem: COpaquePointer?) {
    if (mem == null) {
        printError("Invoked object destructor with null as argument", "kotlin destructor", "Entry.kt", 0)
        exitProcess(EXIT_FAILURE)
    }
...
inline fun <reified T: godot.GodotObject> invoke(name: String, ret: COpaquePointer?, objRaw: COpaquePointer?, numArgs: Int, args: COpaquePointer?, shedule: (T, Int, CPointer<COpaquePointerVar>?) -> Variant?) {
    if (objRaw == null) {
        printError("Invoked method $name for null object", "$name invocation", "Entry.kt", 0)
        exitProcess(EXIT_FAILURE)
    }
    ...
    try {
        ...
        printError(e.message.toString(), name, obj.toString(), 0)
        e.printStackTrace()
        exitProcess(EXIT_FAILURE)
    }
}
fun noMethodToInvoke(name: String, className: String, numArgs: Int) {
    printError("No such method \"$name\" in $className with $numArgs arguments!", name, "", 0) // TODO: ???
    exitProcess(EXIT_FAILURE)
}
...
inline fun <reified T: godot.GodotObject> set(name: String, cl: String, objRaw: COpaquePointer?, v: COpaquePointer?, shedule: (T, Variant) -> Unit) {
    if (objRaw == null) {
        printError("Invoked $name setter for null of $cl", "$name setter", "Entry.kt", 0)
        exitProcess(EXIT_FAILURE)
    }
    if (v == null) {
        printWarning("Invoked setter for $name in $cl with null as param", "$name setter", "Entry.kt", 0)
        return
    }
    val obj = objRaw.asStableRef<T>().get()
    val value = Variant(v)
    try {
        ...
        printError(e.message.toString(), "$name setter", obj.toString(), 0)
        e.printStackTrace()
        exitProcess(EXIT_FAILURE)
    }
}
...
inline fun <reified T: godot.GodotObject> get(name: String, cl: String, objRaw: COpaquePointer?, ret: COpaquePointer?, shedule: (T) -> Variant?) {
    if (objRaw == null) {
        printError("Invoked $name getter for null of $cl", "$name getter", "Entry.kt", 0)
        exitProcess(EXIT_FAILURE)
    }
    val obj = objRaw.asStableRef<T>().get()
    try {
        ...
        printError(e.message.toString(), "$name getter", obj.toString(), 0)
        e.printStackTrace()
        exitProcess(EXIT_FAILURE)
    }
}